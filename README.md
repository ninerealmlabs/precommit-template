...
Hooks are declared in a `.pre-commit-config.yaml`, which is read by both [prek](https://github.com/j178/prek) (the
default runner) and [pre-commit](https://pre-commit.com/) itself.
...
The documentation site publishes machine-readable context so an agent can apply and maintain this template
