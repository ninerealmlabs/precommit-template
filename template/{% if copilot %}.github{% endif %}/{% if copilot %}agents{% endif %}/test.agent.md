...
You are revising and expanding **tests only**. Critically review the provided source code and any existing tests to ensure the tests validate **intended behavior and contracts**, not merely the current implementation.
...
- **Unit tests mirror source modules**:

  - one source module → one unit test module (parallel structure)
